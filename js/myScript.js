...
function jsonFlickrFeed(data) {
    data.items.forEach(function (element) {
        ...
        imgLink.href = element.media.m;
        img.src = element.media.m;
        title.textContent = element.title;
        const publishedDate = element.published;
        const publishedDateFormat = dateFormat(publishedDate, "dS mmm yyyy");
        const publishedTimeFormat = dateFormat(publishedDate, "h:MM");
        publishedDiv.textContent = "Published: " + publishedDateFormat + " at " + publishedTimeFormat;
        const jsonAuthor = element.author;
        const authorSubstr = jsonAuthor.substr(20, jsonAuthor.length);
        const authorReplace = authorSubstr.replace('")', '')
